...
source('compile.se.month.specification.R')

quantile=.95; quant=".95" #just type it twice OK
#month="jun"
lat.start=1
n.lat=5268 
n.lon=4823
...
for (i in 1:n.lon){
  ...
  se=m[,29];
  d=data.frame(d,se) #add this lon's coef files to a growing data frame 'd' of values
  setTxtProgressBar(pb, i)
}
d=d[,-1] #delete the first column of NA's you used to initialize it

#Output the resulting data frame to new .csv file
out.name=paste("/scratch/users/tballard/tmin.extracts/cleaned/tmin.",month,".se",quant,".csv",sep="")
